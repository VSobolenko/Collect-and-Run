    ...
    where TOut : StorePurchaseRequestResult<TIn>, new()
{
    ...
    protected async Task<TOut> ConfirmPurchaseOnServer(string transactionId, string receipt, CancellationToken token)
    {
        ...
        // TODO: replace with a real backend call when purchase validation endpoint is ready.
        var result = new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent("{\"message\":\"Fake Server\"}"),
        };
        ...
        return await Task.FromResult(new TOut()
        {
            ...
            message = ExtractMessage(responseJson),
        });
    }
    ...
    private static string ExtractMessage(string responseJson)
    {
        if (string.IsNullOrWhiteSpace(responseJson))
            return string.Empty;

        const string key = "\"message\":\"";
        var startIndex = responseJson.IndexOf(key, StringComparison.Ordinal);
        if (startIndex < 0)
            return responseJson;

        startIndex += key.Length;
        var endIndex = responseJson.IndexOf('"', startIndex);

        return endIndex > startIndex ? responseJson.Substring(startIndex, endIndex - startIndex) : responseJson;
    }
}
}